...
| Rollback meta-failure error boundary | `RecoveryNote` + propagate `ExecutionError` caught silently by tool | Never swallow rollback errors; RecoveryNote gives operator manual instructions. Tool's primary error (PR creation) is returned; meta-failure surfaced via note. |
| Commit conditional on to_neutralize | Skip commit+push when `to_neutralize` is empty | §8 Explicit: do not make a pointless empty neutralization commit. Push still happens for the regular branch commits. |
| Dirty-tree preflight as root-cause fix for Failure B | `is_clean()` preflight, NOT skip_paths | After a clean-tree preflight there are no untracked files; `git add .` in `commit_with_scope` cannot consume unexpected files. skip_paths is not needed and would incorrectly unstage the artifacts that `neutralize_to_base` staged. |
| LoD structural tests method | Mock-assertion via `tool.execute()` | §14 Test via Public API: test observable collaborator contract through the public entry point. After refactor, `spec=GitManager` mocks make calling illegal methods impossible; `assert_not_called()` pins the boundary. Source inspection tests (inspect.getsource) are preserved for existing ones but no new ones are added. |
...
def prepare_submission(
    ...
    Steps (in order):
        ...
        4. Neutralize (conditional — only when to_neutralize is not empty):
                       neutralize_to_base(to_neutralize, base)
        5. Commit    (conditional — only when to_neutralize is not empty):
        ...
        6. Push      — push() [always executes]
                       On failure:
                         if commit was made → hard_reset("HEAD~1") + RecoveryNote + re-raise
                         else              → RecoveryNote + re-raise (nothing local to undo)

    Design note on Failure B: the is_clean() preflight (step 1) guarantees no untracked
    files exist at commit time. This is the root-cause fix. skip_paths is intentionally
    NOT used — it would incorrectly unstage the artifacts that neutralize_to_base staged.
...
```
GitManager.prepare_submission(artifact_paths=frozenset(), base, ctx)
  ├─ adapter.is_clean() → True
  ├─ adapter.has_upstream() → True
  ├─ for path: has_net_diff_for_path → [] (to_neutralize is empty)
  ├─ [neutralize skipped]
  ├─ [commit skipped]
  └─ push()                           ← push still happens (commits may already be on branch)
...
GitManager.prepare_submission(...)
  ├─ adapter.is_clean() → True
  ├─ adapter.has_upstream() → True
  ├─ to_neutralize: empty  [no artifacts had net diff]
  ├─ [commit skipped — no neutralization]
...
SubmitPRTool: catches ExecutionError in inner try/except → ignores (RecoveryNote already produced)
             → returns ToolResult.error(str(create_pr_exc))

Local branch is in pre-submit state. Remote is stuck at push. Operator must run:
...
| `test_failure_c_meta_rollback_failure_surfaced_via_recovery_note` | `create_pr` raises, `rollback_push` raises | `result.is_error=True` (primary create_pr error); `RecoveryNote` in context (from rollback_push); `set_pr_status` not called |
...
Tests call `tool.execute(params, context)` with a `MagicMock(spec=GitManager)`. Because `spec=GitManager` only exposes real public methods, any call to a method that does not exist on `GitManager` raises `AttributeError`. `assert_not_called()` on specific methods pins the LoD boundary.
...
| `test_submit_pr_execute_does_not_access_adapter` | `MagicMock(spec=GitManager)` (spec enforces: no `.adapter` attribute if not in spec) | `execute()` completes without `AttributeError`; the spec contract makes adapter access impossible |

**Note:** The existing `test_submit_pr_tool_execute_has_no_adapter_calls` (source inspection) is retained as an additional architectural guardrail. No new source-inspection tests are added.
...
| D (no commit variant) — push failed, no commit | `RecoveryNote` | `"Push failed: {reason}. No local commit to roll back. Working tree is clean. Retry submit_pr after resolving the remote issue."` |
