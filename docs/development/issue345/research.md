...
|---|---|---|---|
| Existing `git_delete_branch` callers | Design-phase supersession: enum-based `mode` parameter, `both` as default | Research direction was preserve-compatibility with `remote=False` default; the design phase superseded this with a deliberate breaking change approved by the user — the repo's normal branch lifecycle expects both local and remote cleanup, and an explicit `mode` enum models that contract more honestly than an implicit bool flag | Introduce `mode: Literal["local", "remote", "both"]` with `both` as the default; treat this as an intentional contract break, not an additive extension; preserve the existing idempotent behavior for missing sides |
...
| `submit_pr` tool input schema | Preserve compatibility | The existing `body` field is already a free markdown string that can carry any structured content including deferred-work sections | Do not change `SubmitPRInput`; all deferred-work and tracking-state content is expressed as additional markdown sections within the existing `body` parameter |
