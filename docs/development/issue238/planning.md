...
| File | Role |
|---|---|
| `mcp_server/tools/issue_tools.py` | Production refactor (C1) |
| `mcp_server/scaffolding/templates/concrete/issue.md.jinja2` | H1 fix (C3) |
| `tests/mcp_server/unit/tools/test_issue_body.py` | Deleted (C2) |
| `tests/mcp_server/unit/tools/test_render_body_scaffold_header.py` | Deleted (C2) |
| `tests/mcp_server/unit/tools/test_issue_tools.py` | Updated to `body: str` (C2) |
...
- C2 depends on C1 — test files that import IssueBody will fail to compile until C1 removes it
...
**Tests:**
- RED: write test asserting `CreateIssueInput` accepts `body: str` and rejects a dict — fails on current code
...
- `grep` for `IssueBody` and `_render_body` across codebase returns zero hits outside files scheduled for C2
- `mypy` and `pylint` pass on `mcp_server/tools/issue_tools.py`

---

### Cycle 2: C2 — Delete and update affected test files

**Goal:** Test suite is fully clean. IssueBody-specific test files deleted; all surviving test files updated to `body: str`. No references to `IssueBody` or `_render_body` remain anywhere in the test suite.
...
**Tests:**
- RED: write test asserting `scaffold_artifact(artifact_type='issue', ...)` output does not contain a markdown H1 line (`^#\s`) — fails on current template
- GREEN: remove `# {{ title }}` line from `markdown_header` block in `issue.md.jinja2`; keep `summary` rendering intact
...
- `mypy` and `pylint` pass; `validate_template` gate passes
...
| Risk | Mitigation |
|---|---|
| Non-test code paths construct `IssueBody` or call `_render_body` indirectly | C1 exit criterion: `grep` for `IssueBody` and `_render_body` across entire codebase returns zero hits outside C2-scheduled files |
| `test_all_tools.py` and `test_github.py` import `IssueBody` — compile errors after C1 before C2 | C1 and C2 committed sequentially; C2 GREEN phase resolves all import/compile errors before quality gates run |
