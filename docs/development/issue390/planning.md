...
<!-- template=planning version=130ac5ea created=2026-06-11T06:35:05Z updated=2026-06-11T08:54Z -->
...
* Migration of all 9 JSON-producing/deliverables tools.
* Central test helper `assert_structured_result` implementation.
* Migration of `GetWorkContextTool` to eliminate context response truncation.
...
The implementation is broken down into 9 sequential cycles to allow for step-by-step TDD verification and minimal regression risk:
...
| **Cycle 3** | GetWorkContextTool Migratie | Eliminates context truncation early. | `discovery_tools.py` | GetWorkContextTool unit tests |
| **Cycle 4** | GetPRTool Migratie | Fourth tool migration. | `pr_tools.py` | GetPRTool unit tests |
| **Cycle 5** | GetProjectPlanTool Migratie | Fifth tool migration. | `project_tools.py` | GetProjectPlanTool unit tests |
| **Cycle 6** | InitializeProjectTool Migratie | Validates multiple inheritance. | `project_tools.py` | InitializeProjectTool unit tests |
| **Cycle 7** | Scaffold Tools Migratie | Scaffold tools (schema & artifact). | `scaffold_schema_tool.py`, `scaffold_artifact.py` | Scaffolder tool tests |
| **Cycle 8** | Schema Validation & Rename | Pydantic validation & cycles renaming. | `project_manager.py`, schemas, templates | Deliverables validation tests |
| **Cycle 9** | Cleanup & QA | Cleanup of tests & gates tools. | `test_tools.py`, `quality_tools.py` | Entire pytest suite |
...
### Cycle 3: GetWorkContextTool Migratie

**Goal:** Migrate GetWorkContextTool to StructuredTool to eliminate context response truncation.

| ID | Deliverable | File |
| :--- | :--- | :--- |
| **D3.1** | Migrated GetWorkContextTool in discovery_tools.py | `mcp_server/tools/discovery_tools.py` |

* **Exit Criteria:** `GetWorkContextTool` unit tests pass.
* **Regression Obligations:** Verify settings parsing and context notes generation are fully preserved.

### Cycle 4: GetPRTool Migratie

**Goal:** Migrate GetPRTool to StructuredTool.

| ID | Deliverable | File |
| :--- | :--- | :--- |
| **D4.1** | Migrated GetPRTool in pr_tools.py | `mcp_server/tools/pr_tools.py` |

* **Exit Criteria:** `GetPRTool` unit tests pass.
* **Regression Obligations:** Protect PR information output formatting.

### Cycle 5: GetProjectPlanTool Migratie

**Goal:** Migrate GetProjectPlanTool to StructuredTool.

| ID | Deliverable | File |
| :--- | :--- | :--- |
| **D5.1** | Migrated GetProjectPlanTool in project_tools.py | `mcp_server/tools/project_tools.py` |

* **Exit Criteria:** `GetProjectPlanTool` unit tests pass.
* **Regression Obligations:** Verify phase plan dictionaries are not mutated.

### Cycle 6: InitializeProjectTool Migratie

**Goal:** Migrate InitializeProjectTool with multiple inheritance.

| ID | Deliverable | File |
| :--- | :--- | :--- |
| **D6.1** | Migrated InitializeProjectTool in project_tools.py | `mcp_server/tools/project_tools.py` |
...
### Cycle 7: Scaffold Tools Migratie

**Goal:** Migrate ScaffoldSchemaTool and ScaffoldArtifactTool to StructuredTool.

| ID | Deliverable | File |
| :--- | :--- | :--- |
| **D7.1** | Migrated ScaffoldSchemaTool in scaffold_schema_tool.py | `mcp_server/tools/scaffold_schema_tool.py` |
| **D7.2** | Migrated ScaffoldArtifactTool in scaffold_artifact.py | `mcp_server/tools/scaffold_artifact.py` |
...
### Cycle 8: Deliverables Schema's & Cycles Validatie

**Goal:** Implement Pydantic validation for deliverables and rename `tdd_cycles` to `cycles`.

| ID | Deliverable | File |
| :--- | :--- | :--- |
| **D8.1** | Pydantic schemas in deliverables.py | `mcp_server/schemas/deliverables.py` |
| **D8.2** | Updated ProjectManager with conditional cycles validation and rename | `mcp_server/managers/project_manager.py` |
...
### Cycle 9: RunTestsTool & RunQualityGatesTool Cleanup

**Goal:** Clean up RunTestsTool and RunQualityGatesTool and verify full test suite.

| ID | Deliverable | File |
| :--- | :--- | :--- |
| **D9.1** | Cleaned up RunTestsTool and RunQualityGatesTool | `mcp_server/tools/test_tools.py`, `mcp_server/tools/quality_tools.py` |
...
* Cycle 3 depends on Cycle 2 (for the test helper pattern).
* Cycles 4, 5, 6, 7 depend on Cycle 3 (benefit directly from truncation-free work contexts).
* Cycle 8 depends on Cycles 1-7 (requires complete codebase readiness before rename cutover).
* Cycle 9 depends on Cycle 8.
...
| 1.2 | 2026-06-11 | Agent | Expanded to 9 cycles, placing GetWorkContextTool as Cycle 3 (second tool to migrate) to reduce implementer friction. |
