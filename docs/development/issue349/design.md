...
Define the technical schema and architecture required to transition to pure declarative template resolution.
...
The scaffolding trinity (Jinja2 templates, artifacts.yaml, Pydantic context schemas) is currently bundled, requiring Python source code modifications for any template overrides. The Python dependency introduces security risks and blocks dynamic workspace-local overrides.
...
**Decision:** Implement Option A (Pydantic create_model). We will completely drop python module lookup for context schemas and replace it with an in-memory builder in ArtifactManager.

**Rationale:** Option A strictly adheres to ARCHITECTURE_PRINCIPLES.md (CQS / Frozen query results, Config-First fail fast validation). Retaining Pydantic models keeps blast radius low for downstream consumers and get_schema(), while fulfilling the Clean Break strategy.
...
class BaseRenderContext(BaseModel):
    ...
    # Base lifecycle fields injected by ArtifactManager
