...
The table below covers all cleanup items. It distinguishes between items that are **tracked in git** (relevant for filter-repo) and **untracked local state** (pycache dirs generated by test runs, not in git history).

**Tracked — must be moved or deleted before filter-repo:**

| Path | Action | Classification |
|---|---|---|
| `tests/unit/config/test_c_loader_structural.py` | Move to `tests/mcp_server/unit/config/` | MCP test (imports `mcp_server.*`) |
| `tests/unit/config/test_c_settings_structural.py` | Move to `tests/mcp_server/unit/config/` | MCP test (imports `mcp_server.*`) |
| `tests/unit/__init__.py` | Delete after moves | Structural only |
| `tests/unit/config/__init__.py` | Delete after moves | Structural only |

After these moves, `tests/unit/` has zero tracked source files. No other tracked files exist under `tests/unit/`, `tests/integration/`, `tests/acceptance/`, or `tests/regression/` as confirmed by `git ls-files`.

**Untracked local pycache subtrees — not in git history, no filter-repo impact:**

These directories exist on disk from previous test runs but are gitignored and not tracked. They do not affect migration. Local cleanup (delete) is optional but recommended for hygiene.

| Subtree | Classification of pycache content |
|---|---|
| `tests/unit/mcp_server/` | MCP tests (git_adapter, github_adapter, label_config, settings, phase_state_engine, etc.) — historically existed here before migration to `tests/mcp_server/` |
| `tests/unit/backend/core/` | ST3 backend tests (phase_detection, scope_encoder) |
| `tests/unit/core/` | ST3 backend tests (enums, eventbus, flow_initiator, strategy_cache) |
| `tests/unit/dtos/`, `tests/unit/managers/`, etc. | Mix — see full listing; no source files in any |
| `tests/integration/mcp_server/` | MCP integration tests (server_lifecycle, server_startup, scaffold_tool_execute_e2e) — historically existed here before migration to `tests/mcp_server/` |

**Conclusion:** The `tests/unit/mcp_server/` and `tests/integration/mcp_server/` pycache content confirms these tests have already been migrated to `tests/mcp_server/`. The `tests/unit/core/` and `tests/unit/backend/` pycache content confirms the corresponding ST3 backend tests live in `tests/backend/`.
...
| `docs/setup/` | phase-gate-mcp only — contains MCP server setup guide (GITHUB_TOKEN, mcp.json VS Code template); no ST3 content |
...
| `temp/` | Delete |
| `docs/setup/` | phase-gate-mcp only — MCP server setup content (GITHUB_TOKEN, mcp.json template) |

---

## Open Questions

All grey-area decisions resolved. `docs/setup/` confirmed as phase-gate-mcp only (MCP server setup content; no ST3 content). No open questions remaining.
...
| 1.1 | 2026-06-07 | Agent | QA NOGO fix: explicit test-directory audit (pycache-only subtrees named + classified + git-tracking confirmed), docs/setup/ destination made explicit (phase-gate-mcp only) |